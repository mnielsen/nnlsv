...
interest in rectified linear units (ReLUs, $max(0,x)$), a type of
artificial neuron which appears to learn faster than other common
types of neuron, such as sigmoid and tanh units. (Krizhevsky et al.
(2012))  Our results confirm that this is often the case, but, as we
shall see, it is also often much harder to find hyper-parameters so
ReLUs learn quickly.  We are able to provide a compelling
visualization demonstrating this, and also to precisely quantify how
much harder it is to optimize with ReLUs.
...
units (ReLUs, Nair & Hinton (2010), Glorot (2010)), and softplus
(Glorot (2010)).  These activation functions were applied throughout
the network.
